...
Meanio.prototype.serve = function(options, callback) {
    ...
    if (defaultConfig.db.url){
      url = defaultConfig.db.url;
      opts = defaultConfig.db;
    }
    else {
      url = defaultConfig.db;
      opts = {};
    }

    var database = mongoose.connect(url, opts, function(err) {
        if (err) {
...
function findModules(callback) {
    ...
    Q.all([searchSource('node_modules'), searchSource('packages')]).done(function() {
